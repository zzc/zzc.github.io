...
class Blueprint {
  constructor (doc) {
    ...
    this.blueprintOffset = parseInt(doc['blueprint-offset'] || 0)
    this.blueprintCrop = parseInt(doc['blueprint-crop'] || 0)
  }
}
...
class UnderConstruction extends Section {
  constructor (doc) {
    super(doc)
  }
}
...
function sectionFactory (token) {
  if (isZzcDoc(token)) {
    ...
    if (doc.start === 'simple') { return new Simple(doc) }
